def compCollar(bhid,ocname,ncname):
    ...
    ocollar = pd.read_csv(ocname,encoding = "ISO-8859-1")
    print('\n***** OLD COLLAR INFO*****')
    ...
    ncollar = pd.read_csv(ncname,encoding = "ISO-8859-1")
    print('\n***** NEW COLLAR INFO*****')
    ...
    print('\n***** DIFFERENT COLLARS *****')
    #     bhid='BHID'
    merge_all = pd.merge(ocollar,ncollar, how='outer', on=bhid,indicator=True,copy=False)
    ...
    print('The list of additional collars was saved as:','"'+nameadd+'"')


    # Differences in Data

    # merge_int is intersection, only where BHID is present on both
    merge_int = pd.merge(ocollar,ncollar, how='inner', on=bhid,indicator=True,copy=False)
    ...
    for i in range(len(cols_o)):
        if merge_int[cols_o[i]].dtype != merge_int[cols_o[i]].dtype:
            print('\nYou have different types of data for:',cols_o[i],'and',cols_n[i])
            ...
            if merge_int[cols_o[i]].dtype == 'object':
                ...
                df_diff.loc[cond,(cols_o[i][:-2]+'_old')] = merge_int.loc[cond,cols_o[i]]
                df_diff.loc[cond,(cols_o[i][:-2]+'_new')] = merge_int.loc[cond,cols_n[i]]
            else:
                cond = (np.abs(merge_int[cols_o[i]] - merge_int[cols_n[i]]) > 0.001)
                df_diff[(cols_o[i][:-2])] =  cond* 1
                df_diff.loc[cond,(cols_o[i][:-2]+'_old')] = merge_int.loc[cond,cols_o[i]]
                df_diff.loc[cond,(cols_o[i][:-2]+'_new')] = merge_int.loc[cond,cols_n[i]]
    ...
    print('The table contains all collars that are matching in both old/new files with the respective fields.')
...
def compSurvey(bhid,at,ocname,ncname):    
    import pandas as pd
    import numpy as np
    import warnings
    ...
    osurvey = pd.read_csv(ocname,encoding = "ISO-8859-1")
    print('\n***** OLD SURVEY INFO*****')
    ...
    nsurvey = pd.read_csv(ncname,encoding = "ISO-8859-1")
    print('\n***** NEW SURVEY INFO*****')
    ...
    merge_all = pd.merge(osurvey,nsurvey, how='outer', on=[bhid,at],indicator=True,copy=False)
    ...
    merge_int = pd.merge(osurvey,nsurvey, how='inner', on=[bhid,at],indicator=True,copy=False)
...
def compAssay(bhid,from_i,to_i,ocname,ncname):
    import pandas as pd
    import numpy as np
    import warnings
    warnings.filterwarnings("ignore")
    ...
    #Assay Import
    oassay = pd.read_csv(ocname,encoding = "ISO-8859-1")
    print('\n***** OLD ASSAY INFO*****')
    ...
    nassay = pd.read_csv(ncname,encoding = "ISO-8859-1")
    print('\n***** NEW ASSAY INFO*****')
    ...
    print('\n***** DIFFERENT ASSAYS *****')
    # outer option means all values will be merged (common ones and different ones)
    merge_all = pd.merge(oassay,nassay, how='outer', on=[bhid,from_i,to_i],indicator=True,copy=False)
    # extract only different values
    df_add = merge_all.loc[merge_all['_merge']!='both',(bhid,from_i,to_i,'_merge')]
    #recreate index from 0
    df_add.reset_index(inplace=True,drop=True)
    #left only and right only are default names from merge function. 
    df_add.replace('left_only','old',inplace=True)
    ...
    #save to .csv
    df_add.to_csv(nameadd,index=False)
    print('There are', len(df_add), 'additional assays found.')
    print('The list of additional assays was saved as:','"'+nameadd+'"')

    # Differences in Data

    # merge_int is intersection, only where BHID, FROM, TO is present on both (inner option)
    merge_int = pd.merge(oassay,nassay, how='inner', on=[bhid,from_i,to_i],indicator=True,copy=False)
    ...
    print('\nA csv table was saved as:','"'+namedif+'"')
    print('The table contains all assays that are matching in both old/new files with the respective fields.')
    print('It is in binary code, being 0 no difference, and 1 presence of difference.')
    print('Whenever a difference exists, both old/new values are shown.')

def validSurvey(bhid,at,brg,dip,filen):
    import pandas as pd
    import numpy as np
    ...
    survey = pd.read_csv(filen,encoding = "ISO-8859-1")
    errordf = pd.DataFrame();
    error_l = np.array([])
    fields = [bhid, at, brg, dip]
    survey = survey[fields]
    cond_inv = survey[(survey[dip] > 90) | (survey[dip] < -90) | (survey[brg] > 360) | (survey[brg] <-360)]
    print('\nValidation 1 - There are',len(cond_inv),'invalid Dip and azimuth values')
    if (len(cond_inv)>0):
    ...
    if (len(cond_dup)>0):
        errordf = errordf.append(survey.loc[cond_dup.index.values],ignore_index=True)
...
def validCollar(bhid,xyzcol,filen):
    ...
    import numpy as np
    #Collar Import
    ncollar = pd.read_csv(filen,encoding = "ISO-8859-1")
    collar = pd.DataFrame()
    ...
    cond_1 = collar[(collar[xyzcol[0]] == 0) | (collar[xyzcol[1]] == 0) | (collar[xyzcol[2]] == 0)]
    print('\nValidation 1 - There are',len(cond_1),'ZERO value on Collar coordinates')
    if (len(cond_1)>0):
        errordf = errordf.append(collar.loc[cond_1.index.values],ignore_index=True)
        error_l = np.append(error_l,len(cond_1)*['Zero Coordinate'])
    
    cond_2 = collar[(collar[xyzcol[0]] % 1 == 0) | (collar[xyzcol[1]] % 1 == 0) | (collar[xyzcol[2]] % 1 == 0)]
    print('\nValidation 2 - There are',len(cond_2),'rounded collar coordinates')
    if (len(cond_2)>0):
        errordf = errordf.append(collar.loc[cond_2.index.values],ignore_index=True)
    ...
    if (len(cond_3)>0):
        errordf = errordf.append(collar.loc[cond_3.index.values],ignore_index=True)
    ...
    if (len(cond_4)>0):
        errordf = errordf.append(collar.loc[cond_4.index.values],ignore_index=True)
        error_l = np.append(error_l,len(cond_4)*['Duplicate Coordinates'])

    cond_5 = collar[(collar[xyzcol[0]] > (collar[xyzcol[1]].mean()-collar[xyzcol[1]].std())) & 
                    (collar[xyzcol[0]] < (collar[xyzcol[1]].mean()+collar[xyzcol[1]].std())) &
                    (collar[xyzcol[1]] > (collar[xyzcol[0]].mean()-collar[xyzcol[0]].std())) & 
                    (collar[xyzcol[1]] < (collar[xyzcol[0]].mean()+collar[xyzcol[0]].std()))] 
    print('\nValidation 5 - There are',len(cond_5),'inverted X and Y')
    if (len(cond_5)>0):
        errordf = errordf.append(collar.loc[cond_5.index.values],ignore_index=True)
...
def validAssay(bhid,from_i,to_i,fields,filen):
    import pandas as pd
    import numpy as np
    import warnings
    warnings.filterwarnings("ignore")
    ...
    assay = pd.read_csv(filen,encoding = "ISO-8859-1")
